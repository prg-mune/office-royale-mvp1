...
function updateCamera() {
  ...
  if (viewMode === "player" && human) {
    targetX = human.x + human.vx * 0.18;
    targetY = human.y + human.vy * 0.18;
  }

  const viewW = WORLD.width / targetZoom;
  const viewH = WORLD.height / targetZoom;
  targetX = clamp(targetX, viewW / 2, WORLD.width - viewW / 2);
  targetY = clamp(targetY, viewH / 2, WORLD.height - viewH / 2);
